...
- WhatsApp KI-KENNZEICHNUNG (EU AI Act Art. 50): jede ausgehende Nachricht läuft durch die lokale `reply()` in POST, NIE direkt über `sendWhatsApp()`. reply() stellt „Hallo, ich bin der digitale Assistent von {Restaurant}." als erste Zeile voran, und zwar bei JEDER Nachricht, nicht nur der ersten (bewusste Entscheidung, geht über die Pflicht hinaus). Wer einen neuen Antwortpfad einbaut, MUSS reply() nutzen: die Prompt-Regel allein reichte nicht, weil LARGE_GROUP, „kein Tisch frei", Storno und alle Fehlermeldungen fest verdrahtete Texte senden statt aiMessage. Die drei Pfade vor dem Restaurant-Load kennen den Namen noch nicht und tragen GENERIC_DISCLOSURE. Der Prompt weist das Modell an, den Satz NICHT selbst zu schreiben; die Regex SELF_ID prüft trotzdem die ersten 120 Zeichen gegen Dopplung (bewusst nur den Anfang, sonst würde ein beiläufiges „KI" mitten im Text die Kennzeichnung verschlucken). Es gibt KEINE Zeitfenster-Logik und kein isNewConversation mehr
- WhatsApp GAST-TEXTE: Anrede „Servus {Name}!", Grußformel „Liebe Grüße / {Restaurant}" unter abschließenden Nachrichten (Bestätigung, Storno, Absage), nicht unter Rückfragen. Datum und Uhrzeit werden IMMER über formatDateDe/formatTimeDe/guests formatiert, nie vom Modell formuliert. Die Reservierungsbestätigung ist komplett deterministisch aus den gespeicherten Daten gebaut und übernimmt den Modelltext NICHT mehr — das Modell hatte Wochentage und Uhrzeiten verwechselt, und genau dieser Satz ist der, auf den sich der Gast verlässt. Keine Emojis (waren vorher drin). `rest.name` immer über die Konstante `restaurantName` (getrimmt), weil mehrere Restaurants ein Leerzeichen am Namensende haben
- ACHTUNG whatsapp_conversations: die Tabelle hat KEIN created_at, nur updated_at, und das ist `timestamp without time zone` — PostgREST liefert es ohne Offset, `new Date()` würde es als Lokalzeit lesen. Aktuell wertet die Route den Zeitstempel nicht mehr aus; wer das wieder tut, muss explizit als UTC parsen
